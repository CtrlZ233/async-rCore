...
impl MemorySet {
    ...
    // /// 添加对共享库代码的映射
    // fn map_share_module(&mut self) {
    //     extern "C" {
    //         fn stest();
    //     }
    //     log::info!("maped share interface va: {:#x}  pa: {:#x}", TRAMPOLINE - 2 * PAGE_SIZE, stest as usize);
    //     self.page_table.map(
    //         VirtAddr::from(TRAMPOLINE - 2 * PAGE_SIZE).into(),
    //         PhysAddr::from(stest as usize).into(),
    //         PTEFlags::R | PTEFlags::X,
    //     );
    // }

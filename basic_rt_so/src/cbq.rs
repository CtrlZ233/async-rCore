
use alloc::sync::Arc;
use alloc::{vec::Vec, vec};
use spin::Mutex;
use lazy_static::*;

use crate::manager::EXECUTOR;
use crate::task::user_task::TaskId;

// ===================== CALLBACK QUEUE =====================

pub const CBQ_VA: usize = 0x8741_0000;
pub const CBQ_VEC_VA: usize = 0x8742_0000;
pub const CAP: usize = 4000 + 5;
// VA 直接转换为 Vec
pub struct CBQueue {
    v_ptr: usize,
    head: usize,
    tail: usize,
    cap: usize,
}

impl CBQueue {
    /// clear tids
    pub fn init() {
        unsafe {
            let mut cbq = &mut *(CBQ_VA as *mut CBQueue);
            cbq.head = 0;
            cbq.tail = 0;
            cbq.v_ptr = CBQ_VEC_VA;
            cbq.cap = CAP;            
        }
    }

    pub fn is_empty(&self) -> bool {
        self.tail - self.head == 0
    }

    pub fn add(&mut self, tid: usize) {
        unsafe {
            let ptr = self.v_ptr as *mut usize;
            *ptr.add(self.tail) = tid;
            self.tail += 1;
        }
    }

    pub fn pop(&mut self) -> Vec<usize> {
        unsafe {
            let mut ret = Vec::new();
            let ptr = self.v_ptr as *mut usize;
            let tail = self.tail;
            for i in self.head..tail {
                ret.push(*ptr.add(i));
            }
            self.head = tail;
            ret   
        }
    }
}

pub fn wakeup_all(pid: usize, tids: &mut Vec<usize>) {
    let mut ex = EXECUTOR[pid].lock();
    let len = tids.len();
    // crate::uprintln!("[user] callback len = {}", len);
    for i in 0..len {
        let tid = TaskId::get_tid_by_usize(tids[i]);
        CBTID.lock().add(tid.get_val());
        ex.wake_coroutine(tid);
        // crate::uprintln!("[user] wake up tid is = {}", tid.get_val());
    }
}


lazy_static! {
    pub static ref CBTID: Arc<Mutex<CBTid>> = Arc::new(Mutex::new(CBTid::new()));
}

pub struct CBTid {
    tids: Vec<bool>,
}

impl CBTid {
    pub fn new() -> Self {
        Self { tids: vec![false; CAP], }
    }

    pub fn add(&mut self, t: usize) {
        self.tids[t] = true;
    }

    pub fn contains_tid(&mut self, t: usize) -> bool {
        // crate::uprintln!("cbtid len is {}", self.tids.len());
        if self.tids[t] == true {
            self.tids[t] = false;
            return true;
        }
        false
    }
}

#[no_mangle]
pub fn check_callback(tid: usize) -> bool {
    CBTID.lock().contains_tid(tid)
}

pub fn cbq_init() {
    CBQueue::init();
}

/// 在内核中向某个用户进程的回调队列中添加协程，因此需要传递 cbq 虚拟地址等信息
pub fn cbq_add(tid: usize, addr: usize, vptr: usize) {
    unsafe {
        let mut cbq = &mut *(addr as *mut CBQueue);
        let ptr = vptr as *mut usize;
        *ptr.add(cbq.tail) = tid;
        cbq.tail += 1;
    }
}
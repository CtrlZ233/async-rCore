...
use crate::task::{user_thread_main, kernel_thread_main};
...
#[no_mangle]
#[inline(never)]
pub fn init_cpu_test(flag: bool) {
    CBQueue::init();
    ...
    CPU.add_thread({
        let thread = if flag {
            Thread::new_box_thread(kernel_thread_main as usize, 1)
        } else {
            Thread::new_box_thread(user_thread_main as usize, 1)
        };
        thread
    });
}
